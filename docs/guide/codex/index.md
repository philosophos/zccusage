...
The `@zccusage/codex` package reuses zccusage's responsive tables, pricing cache, and token accounting to analyze OpenAI Codex CLI session logs.
...
pnpm dlx @zccusage/codex --help
pnpx @zccusage/codex --help
...
Since `npx @zccusage/codex@latest` is quite long to type repeatedly, we strongly recommend setting up a shell alias for convenience:
