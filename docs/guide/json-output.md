...
zccusage supports structured JSON output for all report types, making it easy to integrate with other tools, scripts, or applications that need to process usage data programmatically.
...
zccusage daily --instances --json

# Filter to specific project
zccusage daily --project my-frontend-app --json
```
...
zccusage daily --json --since 20250525 --until 20250530

# Different cost calculation modes
zccusage monthly --json --mode calculate
zccusage session --json --mode display

# Sort order
zccusage daily --json --order asc

# With model breakdown
zccusage daily --json --breakdown

# Project analysis
zccusage daily --json --instances                    # Group by project
zccusage daily --json --project my-project           # Filter to project
zccusage daily --json --instances --project my-app   # Combined usage
```
...
zccusage includes built-in jq processing with the `--jq` option. This allows you to process JSON output directly without using pipes:

```bash
# Get total cost directly
zccusage daily --jq '.totals.totalCost'

# Find the most expensive session
zccusage session --jq '.sessions | sort_by(.totalCost) | reverse | .[0]'

# Get daily costs as CSV
zccusage daily --jq '.daily[] | [.date, .totalCost] | @csv'

# List all unique models used
zccusage session --jq '[.sessions[].modelsUsed[]] | unique | sort[]'

# Get usage by specific date
zccusage daily --jq '.daily[] | select(.date == "2025-05-30")'

# Calculate average daily cost
zccusage daily --jq '[.daily[].totalCost] | add / length'
```
...
zccusage daily --json --since $(date -d '7 days ago' +%Y%m%d) | jq '.summary.totalCostUSD'

# List all unique models used
zccusage session --json | jq -r '.data[].models[]' | sort -u

# Find the most expensive session
zccusage session --json | jq -r '.data | sort_by(.costUSD) | reverse | .[0].session'

# Get daily costs as CSV
zccusage daily --json | jq -r '.daily[] | [.date, .totalCost] | @csv'

# Analyze project costs
zccusage daily --instances --json | jq -r '.projects | to_entries[] | [.key, (.value | map(.totalCost) | add)] | @csv'

# Find most expensive project
zccusage daily --instances --json | jq -r '.projects | to_entries | map({project: .key, total: (.value | map(.totalCost) | add)}) | sort_by(.total) | reverse | .[0].project'

# Get usage by project for specific date
zccusage daily --instances --json | jq '.projects | to_entries[] | select(.value[].date == "2025-05-30") | {project: .key, usage: .value[0]}'
```
...
result = subprocess.run(['zccusage', 'daily', '--json'], capture_output=True, text=True)
...
result = subprocess.run(['zccusage', 'daily', '--instances', '--json'], capture_output=True, text=True)
...
const output = execSync('zccusage session --json', { encoding: 'utf-8' });
...
// Project analysis example
const projectOutput = execSync('zccusage daily --instances --json', { encoding: 'utf-8' });
